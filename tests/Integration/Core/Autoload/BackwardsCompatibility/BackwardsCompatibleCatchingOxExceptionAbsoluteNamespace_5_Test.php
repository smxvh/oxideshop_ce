...
class BackwardsCompatibleCatchingOxExceptionAbsoluteNamespace_5_Test extends \PHPUnit_Framework_TestCase
{
    ...
    public function testBackwardsCompatibleCatchingOxExceptionAbsoluteNamespace()
    {
        ...
        try {
            ...
            $this->assertTrue(true, 'The given exception (oxNew(\OxidEsales\Eshop\Core\Exception\StandardException::class)) was caught');
        } catch (\Exception $exception) {
            /** If the exception got not caught before, the test has failed */
            $this->fail('The given exception (oxNew(\OxidEsales\Eshop\Core\Exception\StandardException::class)) was not caught');
        }
    }
}
